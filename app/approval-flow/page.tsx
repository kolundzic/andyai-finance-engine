export const metadata = { title: "AndyAI Finance Engine — Approval Flow", description: "AndyAI Finance Engine public proof layer" };
...
export default function Page() {
  return (
    ...
    <section style={{maxWidth:1180,margin:"0 auto",padding:"72px 20px 28px"}}>
      ...
      <h1 style={{fontSize:"clamp(42px,8vw,86px)",lineHeight:0.92,letterSpacing:"-0.08em",margin:"12px 0 18px",maxWidth:1000}}>Human approval remains above automation — now shown visually.</h1>
      <p style={{color:"#cbd5e1",fontSize:20,lineHeight:1.8,maxWidth:920}}>The strongest differentiator is not speed alone, but a clear stop point before anything is trusted or used.</p>
    </section>
    <section style={{maxWidth:1180,margin:"0 auto",padding:"20px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:16}}><section style={{border:"1px solid rgba(148,163,184,0.18)",background:"rgba(15,23,42,0.72)",borderRadius:24,padding:20,display:"grid",gap:14}}>
      <div style={{position:"relative",overflow:"hidden",borderRadius:18,border:"1px solid rgba(148,163,184,0.14)",background:"#0b1120"}}>
        <img src="/demo/human-approval-workflow-in-finance-dashboard.png" alt="Approval workflow proof" style={{display:"block",width:"100%",height:"auto"}} />
      </div>
      <div>
        <h3 style={{margin:"0 0 8px",fontSize:24,letterSpacing:"-0.04em"}}>Approval workflow proof</h3>
        <p style={{margin:0,color:"#cbd5e1",lineHeight:1.75,fontSize:16}}>The workflow visualization reinforces the blocked-until-reviewed principle for finance teams and enterprise stakeholders.</p>
      </div>
    </section>
<section style={{border:"1px solid rgba(148,163,184,0.22)",background:"rgba(15,23,42,0.72)",borderRadius:24,padding:20}}>
      <h3 style={{marginTop:0,marginBottom:10,fontSize:24,letterSpacing:"-0.04em"}}>Approval moment</h3>
      <p style={{margin:0,color:"#cbd5e1",lineHeight:1.75,fontSize:16}}>Input received → model drafted → assumptions flagged → human reviews → approved output delivered. The blocked gate is the trust signal.</p>
    </section>
<section style={{border:"1px solid rgba(148,163,184,0.22)",background:"rgba(15,23,42,0.72)",borderRadius:24,padding:20}}>
